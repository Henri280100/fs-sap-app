...
module.exports = async (srv) => {
  ...
  srv.after(["READ"], RootEntities, async (response) => {
    ...
    if (
      ...
      response.contact.addressLabel === null
    ) {
      //Requesting the
      const contact = await SELECT.one.from(
        Contacts,
        response.contact.ID,
        (contact) => {
          contact.ID,
    ...
    if (
      response.hasOwnProperty("chartEntities") &&
      response.chartEntities.length > 0
    ) {
      ...
      if (
        !response.chartEntities[0].hasOwnProperty("forecastValue") ||
        !response.chartEntities[0].hasOwnProperty("targetValue")
      ) {
        ...
        const chartEntities = new Map(
          (
            await SELECT.from(ChartDataEntities, (item) => {
              item.ID, item.forecastValue, item.targetValue;
            }).where({ parent_ID: response.ID })
          ).map((key) => [
            key.ID,
            { forecastValue: key.forecastValue, targetValue: key.targetValue },
          ])
        );
  ...
  srv.before("NEW", RootEntities, async (req) => {
    req.data.contact_ID = (
      await SELECT.one.from(Contacts, (contact) => {
        contact.ID;
      })
    ).ID; //Default Contact to prevent Error when creating address label
  ...
  srv.on("changeCriticality", RootEntities, async (req) => {
    ...
    return UPDATE(RootEntities, headerID).with({
  ...
  srv.on("resetEntities", async (req) => {
    ...
    const unitOfMeasures = await SELECT.from(UnitOfMeasureCodeList).columns(
      "code"
    );
    ...
    for (let i = 0; i < countRootEntities; i++) {
      ...
      rootEntities.push({
        ...
        fieldWithCriticality: determineFieldWithCriticalityValue(
          i === criticaityCodes.length ? criticaityCodes[0] : criticaityCodes[i]
        ),
        integerValue: 20 + 2 * i,
